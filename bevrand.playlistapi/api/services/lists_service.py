from api.services.mapper import ObjectMapper
...
class ListsService:
    ...
    def post_new_list(self, posted_user, user_name, playlist_name):
        ...
        mongo_users = UsersDb(api.mongo.db)
        mongo_object = ObjectMapper.map_json_to_object(posted_user, user_name, playlist_name)
        self.validate_user_is_not_reserved_keywords(mongo_object.username)
        mongo_users.check_if_userlist_exists(mongo_object.username, mongo_object.playlist)
        post_model = mongo_object.to_dict()
        mongo_users.insert_new_list(post_model)
    ...
    def update_list(self, user_name, list_name, json_body):
        ...
        mongo_object = ObjectMapper.map_json_to_object(json_body, user_name, list_name)
        self.validate_user_is_not_reserved_keywords(mongo_object.username)
