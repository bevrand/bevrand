...
from project.services import logic_for_users_controller_get, \
...
from project.services import data_validator
import json
...
def front_page():
    ...
    if list_name is not None:
        validation = data_validator.validate_json_for_list(list_name)
...
def specific_list_for_specific_user():
    ...
    if user_list is not None:
        validation = data_validator.validate_json_for_list(user_list)
        valid = validation['valid']
        if not valid:
            return str(validation['errors']), 400
...
def post_user():
    ...
    validation = data_validator.validate_json_for_post(json_body)
    valid = validation['valid']
    if not valid:
        return str(validation['errors']), 400
...
def remove_user_list():
    ...
    if to_remove_list is not None:
        validation = data_validator.validate_json_for_list(to_remove_list)
        valid = validation['valid']
        if not valid:
            return str(validation['errors']), 400
...
def update_user_list():
    ...
    if old_user is not None:
        validation = data_validator.validate_json_for_user(old_user)
        valid = validation['valid']
        if not valid:
            return str(validation['errors']), 400

    if old_list is not None:
        validation = data_validator.validate_json_for_list(old_list)
        valid = validation['valid']
        if not valid:
            return str(validation['errors']), 400
